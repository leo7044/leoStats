...
AND
IF (_AllianceId > -1, _AllianceId=a.AllianceId, true)
AND
IF
(
...
CREATE PROCEDURE `getPlayerBaseData` (IN `_WorldId` INT, IN `_AccountId` INT, IN `_OwnAccountId` INT)  NO SQL
...
JOIN relation_player p ON p.WorldId=pl.WorldId AND p.AccountId=pl.AccountId
JOIN relation_alliance a ON a.WorldId=p.WorldId AND a.AllianceId=p.AllianceId
LEFT JOIN alliance al ON al.WorldId=pl.WorldId AND al.AllianceId=p.AllianceId AND al.Zeit=pl.Zeit
WHERE pl.WorldId=_WorldId
AND pl.AccountId=_AccountId
AND
IF
(
	(SELECT _OwnAccountId IN (SELECT l.AccountId FROM login l WHERE l.IsAdmin=true)),
